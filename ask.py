...
class ChunkProcessor:
    def __init__(self, llm, max_concurrent: int = MAX_CONCURRENT_REQUESTS):
        self.llm = llm
        self.semaphore = asyncio.Semaphore(max_concurrent)
        self.executor = ThreadPoolExecutor(max_workers=max_concurrent)
        
    async def process_chunk(self, chunk: str, function_schema: dict) -> List[dict]:
    ...
    async def process_chunks_batch(self, chunks: List[str], function_schema: dict) -> List[List[dict]]:
        """Process a batch of chunks concurrently."""
        tasks = [self.process_chunk(chunk, function_schema) for chunk in chunks]
...
async def extract_companies_from_text_async(article_text: str, llm) -> List[dict]:
    """Extract companies from text using optimized parallel processing."""
    try:
        ...
        for i in range(0, total_chunks, BATCH_SIZE):
            ...
            batch_results = await processor.process_chunks_batch(batch, EXTRACTION_FUNCTION)
            results.extend(batch_results)
            logger.info(f"Processed batch {i//BATCH_SIZE + 1}/{(total_chunks + BATCH_SIZE - 1)//BATCH_SIZE}")
        
        # Merge and deduplicate results
        companies = list(chain.from_iterable(results))
        unique_companies = []
        seen = set()
...
async def verify_companies_from_text_async(article: str, company_list: List[dict], llm) -> List[dict]:
    ...
    try:
        ...
        for i in range(0, total_chunks, BATCH_SIZE):
            ...
            # Create verification tasks for the batch
            tasks = []
            for chunk in batch:
                task = processor.process_chunk(
                    chunk,
                    {
                        "name": "verify_companies",
                        "description": "Verify company mentions in the text",
                        "parameters": VERIFICATION_FUNCTION["parameters"]
                    }
                )
                tasks.append(task)
